...
Partial Public Class tmovimento3
    ...
    Private Sub storyboard_saltar(ByRef st As Storyboard, ByVal direcao As Integer)
        ...
        Dim bgleft As Double = bg.GetValue(Canvas.LeftProperty)
        Dim canvasleft As Double = LayoutRoot.GetValue(Canvas.LeftProperty)
        ' If ((bgleft < 0) And (Math.Abs(bgleft) + LayoutRoot.ActualWidth) < bgimg.ActualWidth) Then
    ...
    Private Function gerarEnimigo(Optional ByVal Left As Double = 0.0R)
        ...
        Dim checkleft As New Image
        Dim checkright As New Image
        checkleft.SetValue(Canvas.LeftProperty, Left - 100.0R)
        checkright.SetValue(Canvas.LeftProperty, Left + 150.0R)

        For Each obj As Image In caixas.Children
            If Not (colisao(nemesis, obj) = 0 And colisao(nemesis, player) = 0 And colisao(checkleft, obj) = 0 And colisao(checkright, obj) = 0) Then
                Return False
            End If
        Next
        For Each img As Image In enimigos.Children
            If Not (colisao(nemesis, img) = 0 And colisao(checkleft, img) = 0 And colisao(checkright, img) = 0) Then
                Return False
            End If
        Next
